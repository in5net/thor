...
import Value, { Complex, List, Number, String } from '../../values';
...
export function int(x: Value): Number {
  ...
  throw new Error('int() expects a number or string');
}
...
export function float(x: Value): Number {
  ...
  throw new Error('float() expects a number or string');
}
...
export function len(value?: Value) {
  ...
  throw new Error('len() must receive a list or complex number');
}
...
export function random(min?: Value, max?: Value) {
  ...
  throw new Error(`random() must receive up to 2 numbers or a list`);
}
...
export function norm(n: Value | Number, min: Value, max: Value): Number {
  if (
    ...
    throw new Error('norm() expects 3 numbers');
...
export function lerp(min: Value, max: Value, norm: Value): Number {
  if (
    ...
    throw new Error('lerp() expects 3 numbers');
...
export function map(
  ...
  if (
    ...
    throw new Error('map() expects 5 numbers');
...
export function clamp(n: Value, min: Value, max: Value): Number {
  if (
    ...
    throw new Error('clamp() expects 3 numbers');
...
export function overlap(
  ...
  if (
    ...
    throw new Error('overlap() expects 4 numbers');
...
export function minmax(a: Value, b: Value): List {
  if (!(a instanceof Number) || !(b instanceof Number))
    throw new Error('minmax() expects 2 numbers');
  return new List([
...
export function gcd(av: Value, bv: Value): Number {
  if (!(av instanceof Number) || !(bv instanceof Number))
    throw new Error('gcd() expects 2 numbers');
  let a = av.value;
  let b = bv.value;
  while (b !== 0) [a, b] = [b, a % b];
  return new Number(a);
